...
against its live index and returns their CURRENT verified work email. Three outcomes:
  ...
  verify:apollo-unverified  No verified email returned (person moved on, small shop with a
                            free-mail address which reveal_personal_emails=False will not
                            return, or no match). Hold these - do not enrol.

Payload rules that are load-bearing (see CLAUDE.md): first_name MUST be included or
bulk_match fuzzy-joins to the wrong person; the 2020 email and the company domain are
passed as extra match hints; a returned email whose domain contradicts the company is
...
def email_fits_company(email, company, site_domain):
    ...
    return len(strong) >= 2 and all(c in stem for c in initials)
...
def main():
    ...
    for i in range(0, len(todo), 10):
        ...
        if dry:
            for d in details:
                print(f"  [dry] {d['email']:44} {d['organization_name'][:30]}")
            continue
        ...
        for (e, rec), d, m in zip(batch, details, matches + [None] * len(batch)):
            ...
            if m and m.get("email") and m.get("email_status") == "verified":
                ...
                if email_fits_company(cand, d["organization_name"], d["domain"]):
                    if cand == e:
            ...
            results[e] = {"id": cid, "verdict": verdict, "new_email": new_email}
