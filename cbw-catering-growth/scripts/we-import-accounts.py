...
if os.path.exists(OUT):
    with open(OUT, newline="", encoding="utf-8") as _f:
        for _r in csv.DictReader(_f):
            DONE_KEYS.add((_r["location"].strip().lower(), _r["zip"]))
for f in ["accounts-master.csv"]:
    with open(os.path.join(DATA, f), newline="", encoding="utf-8") as fh:
        for r in csv.DictReader(fh):
            key = (r["Location"].strip().lower(), str(r["Zip Code"]).strip())
            if key in seen or (r["Location"].strip().lower(), str(r["Zip Code"]).split(".")[0]) in DONE_KEYS:
                continue
            seen.add(key)
            rows.append(r)
...
for acc_id, r in created:
    by_label["brand:wildeggs"].append(acc_id)
