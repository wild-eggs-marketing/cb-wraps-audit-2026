...
const ANSWERS_DEFAULT: Answer[] = [
    ...
    },
    {
        ...
        body: "Vegan as it comes: Banana, Broccoli with Olive Oil & Herb, Carrots, Chips, Garlic Ginger Edamame, Mixed Veggies, Salt & Lime Edamame, Spicy Edamame, Teriyaki Edamame and Vegetable Wontons. Every item on our menu can also be ordered without the grilled chicken it's built with by default — or with a plant protein instead: Tofu (200 cal, 17g protein) or Plant Based Chicken (130 cal, 20g protein). Ordered that way High-Protein Bowl, Stir Fry Bowl, Sweet & Sour Bowl, Teriyaki Bowl and Teriyaki Wrap contain no animal ingredients — on the Stir Fry and High-Protein Bowls, also pick a plant-based sauce, since the Thai peanut sauce contains honey and the pesto contains milk and eggs. The calories and protein shown are measured with the grilled chicken in, so they'll be lower without it. 11 further items are vegetarian as served rather than vegan, including all three of our salads. Bowls with cheese, dairy sauces or honey — the Fajita and Power Bowls (cheese), the Mediterranean, Pesto and Jerk Bowls (dairy sauces), and the Thai Bowl (honey) — are vegetarian without the chicken rather than vegan.",
